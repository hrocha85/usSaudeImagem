...
function TenComumExtensoresAntebracoEsquerdo({ Disable }) {
    ...
    const [ConclusaoTenComumExtensoresAntebracoEsquerdo, setConclusaoTenComumExtensoresAntebracoEsquerdo] = useState<any>([]);

    const subExame = 'Cotovelo- Tendão comum extensores antebraço '
    const titulo_exame = 'Articulações'

    useEffect(() => {
        if (Object.keys(fraseTenComumExtensoresAntebracoEsquerdo).length === 0) {
            new Format_Laudo(
                titulo_exame,
                subExame,
                true,
                fraseTenComumExtensoresAntebracoEsquerdo,
                ConclusaoTenComumExtensoresAntebracoEsquerdo
    ...
    const removeRoturaParcial = () => {
        fraseTenComumExtensoresAntebracoEsquerdo.map((e) => {
            if (e.includes("Espessado, com alteração ecotextural, observando-se sinais de rotura parcial medindo")) {
                const index = fraseTenComumExtensoresAntebracoEsquerdo.indexOf(e);

                if (index > -1) {
                    fraseTenComumExtensoresAntebracoEsquerdo.splice(index, 1);
                    setFraseTenComumExtensoresAntebracoEsquerdo((arr) => [...arr]);
                }
            }
        });
    };
    ...
    const criaStringPequenasCalcificacoes = () => {
        ...
        if (PequenasCalcificacoesCheckbox) {
            setFraseTenComumExtensoresAntebracoEsquerdo((arr) => [...arr, string])
            setConclusaoTenComumExtensoresAntebracoEsquerdo((arr) => [...arr, conclusao])
        } else {
    ...
    const removeItemString = (value) => {
        const index = fraseTenComumExtensoresAntebracoEsquerdo.indexOf(value);
        if (index > -1) {
            fraseTenComumExtensoresAntebracoEsquerdo.splice(index, 1);
            setFraseTenComumExtensoresAntebracoEsquerdo((arr) => [...arr]);
        }
    ...
    const removeItemConclusao = (value) => {
        const index = ConclusaoTenComumExtensoresAntebracoEsquerdo.indexOf(value);
        if (index > -1) {
            ConclusaoTenComumExtensoresAntebracoEsquerdo.splice(index, 1);
            setConclusaoTenComumExtensoresAntebracoEsquerdo((arr) => [...arr]);
    ...
    return (
        <Box
            ...
            mt="15px"
        >
            <TituloNomeExame titulo="Tendão comum dos extensores do antebraço Esquerdo" />
...
export default TenComumExtensoresAntebracoEsquerdo;
