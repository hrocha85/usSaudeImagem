...
function SupraEspinhalOmbroEsquerdo() {
  const altura = "100%";
  const largura = "95%";
  ...
  const criaStringTendinite = (medida, Tendinite) => {
    removeTendinite();
    if (medida !== "") {
      var string = `Ombro esquerdo com Tendinite Supraespinhal ${Tendinite} com calcificação de ${medida} mm `;
      setLaudoPrin((arr) => [...arr, string]);
    }
  };
  ...
  const removeTendinite = () => {
    laudoPrin.map((e) => {
      if (e.includes("Ombro esquerdo com Tendinite Supraespinhal")) {
  ...
  const criaStringRotura = (medida, Rotura) => {
    removeRotura();
    if (medida !== "") {
      var string = `Ombro esquerdo com Rotura Supraespinhal ${Rotura} com intervalo de ${medida} mm `;
      setLaudoPrin((arr) => [...arr, string]);
    }
  };
  ...
  const removeRotura = () => {
    laudoPrin.map((e) => {
      if (e.includes("Ombro esquerdo com Rotura Supraespinhal")) {
        var index = laudoPrin.indexOf(e);
...
export default SupraEspinhalOmbroEsquerdo;
