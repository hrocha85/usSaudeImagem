...
function InfraespinhalOmbroEsquerdo() {
  const altura = "100%";
  const largura = "95%";
  ...
  const criaStringTendinite = (medida, Tendinite) => {
    removeTendinite();
    if (medida !== "") {
      var string = `Ombro Esquerdo com Tendinite Infraespinhal ${Tendinite} com calcificação de ${medida} mm `;
      setLaudoPrin((arr) => [...arr, string]);
    }
  };
  ...
  const removeTendinite = () => {
    laudoPrin.map((e) => {
      if (e.includes("Ombro Esquerdo com Tendinite Infraespinhal")) {
  ...
  const criaStringRotura = (medida, Rotura) => {
    removeRotura();
    if (medida !== "") {
      var string = `Ombro Esquerdo com Rotura Infraespinhal ${Rotura} com intervalo de ${medida} mm `;
      setLaudoPrin((arr) => [...arr, string]);
    }
  };
  ...
  const removeRotura = () => {
    laudoPrin.map((e) => {
      if (e.includes("Ombro Esquerdo com Rotura Infraespinhal")) {
        var index = laudoPrin.indexOf(e);
  ...
  return (
    <Box
      ...
      mt="15px"
    >
      <TituloNomeExame titulo="Infraespinhal" />
...
export default InfraespinhalOmbroEsquerdo;
