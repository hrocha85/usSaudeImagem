...
function RimEsquerdo() {
  ...
  const [frasesRimEsquerdo, setFrasesRimEsquerdo] = useState<any>([]);
  ...
  const criarString = (value, valueId?, valueInput?) => {
    ...
    setFrasesRimEsquerdo((arr) => [...arr, value]);
    //console.log("criaString = ", laudoPrin)
  };

  const removeItemString = (value) => {
    // console.log("valor remove = ", value);
    const index = frasesRimEsquerdo.indexOf(value);
    //caso o valor enviado exista no array, vai remover com splice e setar array novamente
    if (index > -1) {
      frasesRimEsquerdo.splice(index, 1);
      setFrasesRimEsquerdo((arr) => [...arr]);
    }
  ...
  const CriaStringMedidas = (value) => {
    ...
    const valorInput =
      ...
      dadoInputParenquima +
      "mm ";
    setFrasesRimEsquerdo((arr) => [...arr, valorInput]);
    setValueFraseMedida(valorInput);
  };

  const removeStringMedida = () => {
    const index = frasesRimEsquerdo.indexOf(valueFraseMedida);
    if (index > -1) {
      frasesRimEsquerdo.splice(index, 1);
      setFrasesRimEsquerdo((arr) => [...arr]);
    }
  ...
  useEffect(() => {
    if (Object.keys(frasesRimEsquerdo).length == 0) {
      new Format_Laudo(
        titulo_exame,
        subExame,
        true,
