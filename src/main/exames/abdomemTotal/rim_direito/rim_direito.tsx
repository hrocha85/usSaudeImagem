...
function RimDireito() {
  ...
  const [frasesRimDireito, setFrasesRimDireito] = useState<any>([]);
  ...
  const criarString = (value, valueId?, valueInput?) => {
    ...
    setFrasesRimDireito((arr) => [...arr, value]);
  ...
  const CriaStringMedidas = (value) => {
    ...
    const valorInput =
      ...
      dadoInputParenquima +
      "mm ";
    setFrasesRimDireito((arr) => [...arr, valorInput]);
    setValueFraseMedida(valorInput);
  };

  const removeStringMedida = () => {
    const index = frasesRimDireito.indexOf(valueFraseMedida);
    if (index > -1) {
      frasesRimDireito.splice(index, 1);
      setFrasesRimDireito((arr) => [...arr]);
    }
  ...
  useEffect(() => {
    if (Object.keys(frasesRimDireito).length == 0) {
      new Format_Laudo(
        titulo_exame,
        subExame,
        true,
